...
parser.add_argument(
    ...
    choices=("prompt_only", "attention_only", "full"),
    default=None,
...
from characonsist.story import parse_prompt_scenes, save_story_visualization
...
def prepare_merge_audit(spatial_kwargs, args):
    ...
    mode = normalize_consistency_mode(
        getattr(args, "consistency_mode", None), getattr(args, "use_interpolate", False)
    )
    if getattr(args, "save_merge_maps", False) and mode == "full":
...
def finalize_merge_audit(spatial_kwargs, out_dir, frame, args):
    ...
    if not getattr(args, "save_merge_maps", False) or mode != "full":
        return None
...
def prepare_component_audit(spatial_kwargs):
    spatial_kwargs["component_audit_state"] = new_component_state()


def finalize_component_audit(spatial_kwargs, mode, frame):
...
def run_prompt_file(pipe, args):
    ...
    if args.mix_mode:
        ...
        if getattr(args, "save_merge_maps", False) and consistency_mode == "full":
